CREATE TABLE codex_run_contexts (
    ...
    adapter TEXT NOT NULL CHECK (adapter = 'codex_exec_json'),
...
CREATE INDEX codex_run_contexts_owner_lookup ON codex_run_contexts(public_reference, project_id, worktree_id, task_key);
CREATE INDEX codex_run_contexts_active_lookup ON codex_run_contexts(project_id, worktree_id, lifecycle);
CREATE UNIQUE INDEX codex_run_contexts_one_active_owner
    ON codex_run_contexts(project_id, worktree_id, task_key)
    WHERE lifecycle IN ('created', 'starting', 'running', 'cancelling');
