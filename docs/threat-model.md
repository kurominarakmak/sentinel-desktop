...
| Repository Git filter | Repository-controlled clean/process helper executes during inspection | AH1 retires production stock-status and fails inventory/cleanliness closed pending a filter-free pipeline | Same-user filesystem races remain; direct-child kill/reap does not itself prove descendant containment |
