...
describe("V3AttentionWidget", () => {
  ...
  it("renders durable state and routes task detail without polling", async () => { render(<V3AttentionWidget />); await screen.findByText("Working"); expect(screen.getByText("Claude · Ship status")).toBeInTheDocument(); await userEvent.click(screen.getByRole("button", { name: "Open Task Detail" })); await waitFor(() => expect(prompt.show).toHaveBeenCalledOnce()); expect(emit).toHaveBeenCalledWith("v3-open-task-detail", "task-1"); expect(listen).toHaveBeenCalledWith("v3-state-changed", expect.any(Function)); await userEvent.click(screen.getByRole("button", { name: "Stop" })); expect(invoke).toHaveBeenCalledWith("cancel_codex_session_task", { request: { taskId: "task-1" } }); });
  it("wires supported stop and approval actions", async () => { invoke.mockImplementation((command: string) => command === "list_v3_tasks" ? Promise.resolve([task("awaiting_approval")]) : Promise.resolve(detail("awaiting_approval", true))); render(<V3AttentionWidget />); await screen.findByText("Approval required"); await userEvent.click(screen.getByRole("button", { name: "Approve" })); expect(invoke).toHaveBeenCalledWith("decide_v3_final_approval", { request: { approvalId: "approval-1", approve: true } }); });
});
