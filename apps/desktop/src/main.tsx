...
const services: Phase2AppServices = { api, approvalApi: approvalApi(invoke), async listenToRunEvents(callback) { return listen<RunEvent>("phase2-run-event", (event) => callback(event.payload)); }, async listenToTaskInputFocus(callback) { return listen("focus-task-input", () => callback()); }, async hidePrompt() { await getCurrentWindow().hide(); } };
