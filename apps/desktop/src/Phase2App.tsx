...
export function Phase2App({ services }: { services: Phase2AppServices }) {
  ...
  return <main onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void run(); } }}><header><strong>Agent Sentinel</strong><span>Desktop prompt</span></header><section><h2>New task</h2><fieldset><legend>Project</legend>{projects.length ? projects.map((item) => <button key={item.id} aria-pressed={projectId === item.id} onClick={() => setProjectId(item.id)}>{item.display_name} · {item.validation_state}</button>) : <p>No registered project</p>}</fieldset><fieldset><legend>Managed worktree</legend>{worktrees.length ? worktrees.map((item) => <button key={item.id} aria-pressed={worktreeId === item.id} onClick={() => setWorktreeId(item.id)}>Ready · {item.id.slice(0, 8)}</button>) : <p>{projectId ? "No ready worktree" : "Choose a project"}</p>}</fieldset><fieldset><legend>Agent</legend>{(["fake", "codex", "claude_code"] as const).map((value) => <button key={value} aria-pressed={agent === value} onClick={() => setAgent(value)}>{value === "fake" ? "Fake agent" : value === "codex" ? "Codex" : "Claude Code"}</button>)}</fieldset><label>Task<textarea ref={input} value={task} onChange={(event) => setTask(event.target.value)} rows={4} /></label><label>Scenario<select value={scenario} onChange={(event) => setScenario(event.target.value as Scenario)} disabled={!fakeSelected}>{scenarios.map((value) => <option key={value}>{value}</option>)}</select></label><button onClick={() => void run()} disabled={!task.trim() || submitting || !environment?.fake_agent_available || !fakeSelected}>{submitting ? "Submitting…" : fakeSelected ? "Run fake agent" : "Selected agent unavailable"}</button>{!fakeSelected && <p role="status">This agent is not available in the desktop workflow yet.</p>}{submissionError && <p role="alert">{submissionError}</p>}</section><section><h2>Availability</h2><p>Fake Agent — {environment?.fake_agent_available ? "available" : "unavailable"}</p><p>Codex — configured execution unavailable</p><p>Claude — configured execution unavailable</p><p>Notifications — {desktopCapabilities?.notifications ? "available" : "unavailable"}; autostart — {desktopCapabilities?.autostart ? "available" : "unavailable"}</p>{environmentError && <><p role="alert">{environmentError}</p><button onClick={() => void refreshEnvironment()} disabled={environmentLoading}>Retry availability</button></>}{liveUpdatesError && <p role="alert">{liveUpdatesError}</p>}</section><ApprovalPanel api={services.approvalApi} projectId={projectId} worktreeId={worktreeId} /><section><h2>Run history</h2>{historyError && <><p role="alert">{historyError}</p><button onClick={() => void refreshRuns()}>Retry history</button></>}{runs.length ? <ol>{runs.map((run) => <li key={run.id}><button onClick={() => void selectRun(run.id, run)}>{run.status} · {run.id.slice(0, 8)} · {run.task_text.slice(0, 60)}</button></li>)}</ol> : <p>No runs yet.</p>}</section><section aria-live="polite"><h2>Run detail</h2>{selected ? <><p>{selected.status} · {selected.id}</p>{selectedRunLoading && <p>Refreshing run details…</p>}{selectedEventsError && <p role="alert">{selectedEventsError}</p>}{selectedMetadataError && <p role="alert">{selectedMetadataError}</p>}{selectedCancellation?.feedback && <p role="alert">{selectedCancellation.feedback}</p>}{(selectedEventsError || selectedMetadataError) && <button onClick={() => void retrySelectedRecovery()} disabled={selectedRunLoading}>Retry</button>}{selected.cancellable && <button onClick={() => void cancel()} disabled={selectedCancellation?.pending}>{selectedCancellation?.pending ? "Cancelling…" : "Cancel"}</button>}<ol>{events.map((event) => <li key={event.sequence_number}>#{event.sequence_number} {event.event_type}: {formatEventSummary(event)}</li>)}</ol></> : <p>Select a run to inspect its persisted events.</p>}</section></main>;
}
