...
import { formatEventSummary, isRunLifecycleEvent, isScenario, isTerminalRunEvent, MAX_TASK_BYTES, mergeRunEvents, patchRunInHistory, reconcile, scenarios, upsertRunInHistory, type CancellationResult, type RunDto, type RunEvent, type RuntimeEnvironment, type Scenario, type Unlisten } from "./phase2";
...
export function Phase2App({ services }: { services: Phase2AppServices }) {
  const input = useRef<HTMLTextAreaElement>(null); const [task, setTask] = useState(""); const [scenario, setScenario] = useState<Scenario>("success");
